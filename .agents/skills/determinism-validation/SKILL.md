...
Follow `docs/VALIDATION.md` for authoritative commands and acceptance criteria. Do not change production behavior to simplify a test, and do not treat a compile or a local run without both GMTL summary lines as a runtime pass; use the documented hosted Windows evidence when local execution is unavailable.
