...
Start from the named current owner and its routed scope in `docs/ARCHITECTURE.md`; preserve unrelated work. Read [the structural audit](references/structural-audit.md) before selecting an extraction. Use `docs/MODULE_OWNERSHIP_MAP.md` for the audited current/proposed boundary and `docs/STRUCTURAL_DECOMPOSITION_PLAN.md` for the prerequisite milestone; neither changes runtime ownership until a validated extraction does.
...
Route new, moved, or reorganized GameMaker resources through `$gamemaker-resource-change`. Route RNG, timing, replay, save/load, or state-equivalence concerns through `$determinism-validation`. Follow `docs/VALIDATION.md` and `docs/AGENT_REVIEW_POLICY.md` for validation and review evidence. Stop when the approved milestone is complete.
