...
1. Establish focused characterization coverage before moving behavior whose contract is not already verified. Prefer existing focused GMTL tests; add the smallest contract test only in the separately authorized characterization task.
...
6. Run focused validation after each meaningful extraction. Use the full relevant validation tier before completion, following `docs/VALIDATION.md`; a compile or missing local GMTL summaries is not runtime evidence.
