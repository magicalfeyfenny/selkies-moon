...
It records the audited current ownership and proposed future seams as of
2026-07-25. Proposed resources do not exist and are not current runtime owners.
The current owners in [Architecture](ARCHITECTURE.md) remain authoritative until
the relevant characterization prerequisite and a validated extraction are both
complete.
...
The current resource owns 176 public function declarations and 152 macros.
...
| Pause menu | Pause state/input, live tuning, state machine, and draw | `obj_UI_menu` Step/Draw; title-owned options; input helpers; story/title-owned draw primitives | Mutates runtime stock/rank/meter/config; freeze/resume and End Step close ordering; config writes | pause/settings/tuning/quit tests and visual captures; gap: draw-state assertions | `scr_pause_menu`; no facade after `scr_ui_ornate` and options seams exist | characterize |
...
| Wave director and enemy catalog | Spawn band/RNG, rosters, instance configuration, live director, stage enemy bullet decoration, combat clear | scene manager, enemy variant, bullet draw, boss patterns, visual tour | Consumes RNG, creates/destroys entities, writes enemy fields, relies on stage/rank and instance Create order | roster/director/balance tests; gap: seeded RNG and ordered spawn/entity trace | `scr_wave_director`; no facade | characterize |
...
| Enemy projectiles | Shared linear spawn/flash/decoration, blade motion and redirects; standard-enemy firing stays event-owned | enemy variant, bullet parent/blade, player, boss patterns/runtime | Consumes RNG, creates/mutates bullets, snapshots rank speed, uses inherited Step guards | projectile geometry/motion/redirect tests; gap: seeded per-frame trace | `scr_enemy_projectiles`; no facade | characterize |
...
- Stage rules, flow, and waves: `GameSceneStateCreate`,
  ...
  `GameStageSpawnBandRectGet`, `GameStageDirectorShouldRun`,
  `GameStageEnemyRosterCreate`,
  `GameStageEnemyDefinitionGet`, `GameEnemyVariantConfigure`,
  `GameStageEnemyBulletDecorate`, `GameStageEnemyBulletSpawn`,
  `GameStageDirectorVariantWaveSpawn`, `GameStageBasicEnemyWaveSpawn`,
...
- Boss plans and runtime: `GameMemoryCorePhaseCreate`,
  ...
  `GameSunsetInfinityOffsetCreate`.
...
- Stage/camera to `scr_stage_rules` or `scr_stage_flow`:
  ...
  `STAGE_SPAWN_SIDE_MARGIN`, `STAGE_COUNT`,
...
- Enemy projectiles and movement to `scr_enemy_projectiles` or
  `scr_wave_director`: `BEAD_BULLET_SPEED`, `DIAMOND_BULLET_SPEED`,
  `BLADE_TURN_SPEED`, `BLADE_RADIAL_SPEED`, `BLADE_TURN_RATE_SCALE`,
  `BLADE_MAX_SCREEN_SPEED`, `BLADE_MAX_RADIAL_SPEED`,
  `BLADE_REDIRECT_MAX_SCREEN_SPEED`.
- Bosses to `scr_boss_plans` or `scr_boss_runtime`:
  ...
  `BOSS_PHASE_REFILL_FRAMES`, `SUNSET_FLOAT_X_RADIUS`,
  `SUNSET_FLOAT_Y_RADIUS`, `SUNSET_FLOAT_RATE`.
...
| Shared ornate UI | `GameUiStoryFramePaletteCreate`, `GameUiDrawOrnamentDiamond`, `GameUiDrawPixelFiligreeCorner`, `GameUiDrawQuadraticThread`, `GameUiDrawFiligreeDivider`, `GameUiDrawVolumeGauge`, `GameUiDrawPixelHeart`, `GameUiDrawBossPhaseHearts`, `GameUiDrawOrnateFrame` | Shared by title, gameplay HUD, pause, story; draw-state side effects | focused palette, caller-state, layout-boundary, and draw-postcondition assertions; 132-test GMTL pass and reviewed selected visual-tour captures; gap: stable pixel baselines | proposed `scr_ui_ornate` | characterization complete; one separately governed bounded extraction may begin |
