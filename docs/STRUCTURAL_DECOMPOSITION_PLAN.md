...
refreshed against the repository foundation on 2026-07-25. It is not
authorization to change production code, GameMaker resources, gameplay, assets,
or release configuration. Each milestone is a separate, bounded task and must
follow [Governance Handoff](GOVERNANCE_HANDOFF.md),
[Agent Review Policy](AGENT_REVIEW_POLICY.md), and [Validation](VALIDATION.md).

The current owners in [Architecture](ARCHITECTURE.md) remain authoritative.
This plan distinguishes their current ownership from proposed future ownership,
and separates characterization prerequisites from later extraction work.
...
| ---: | --- | ---: | --- | ---: | ---: | --- | --- | --- | --- |
| 1 | `Selkie's Moon ~ until we meet again ~/scripts/scr_gameplay_helpers/scr_gameplay_helpers.gml` | 3,836 | `scr_gameplay_helpers`: run flow, rank, stages, players, encounters, pickups, enemies | 14 domain clusters plus a cross-domain balance report; 176 functions and 152 macros | Historical audit found 44 direct external GML references; refresh the exact caller set in each milestone | Co-owns `global.game_runtime`, mutates `global.game_save`, many player/enemy/boss instances, rooms, and entity populations; active in nearly every gameplay Step | Extreme; unrelated gameplay tasks repeatedly load constants, UI, plans, spawning, combat, and rewards together | Extreme: shared RNG stream, frame order, initialization, persistence calls, instance mutation, and balance coupling | Extreme; domain seams can remove most unrelated context from routine work |
| 2 | `Selkie's Moon ~ until we meet again ~/scripts/scr_setup/scr_setup.gml` | 812 | `scr_setup`: schemas, defaults, persistence, display, boot | 6: defaults/runtime schema, persistence transport, migration/validation, result tables, display application, boot | Historical audit found 11 direct external GML references; refresh the exact caller set in each milestone | Owns `global.game_config`, `global.game_save`, `global.game_runtime`, and delayed window globals; boot-critical and save-critical | High despite moderate size because every schema, save, run-end, config, and display task crosses the file | Extreme: circular dependency with gameplay helpers, save compatibility, boot order, file I/O, and delayed window timing | High; separating persistence, runtime schema, results, and display makes ownership and validation much narrower |
...
`test_bootstrap.gml` is approximately 2,991 lines but is not a production target.
...
- all 152 macros live at the top of the file even though they belong to camera,
...
clusters. The retired stage timeline is not a live dependency; the code-driven
director is the current wave source. Room, object-event, resource, and Included
File dependencies remain material and are listed in the module map.
...
- Timeline/sequence/alarm dependencies: the retired stage timeline is not a
  live dependency; no live sequence or alarm call was found in the audited
  clusters.
...
Direct function references in GMTL currently cover 94 of 176 gameplay-helper
